...
// const AddTask = ({ onTaskAdded, user, tasks = [] }) => {
...
//   const checkEnergyAllowance = () => {
//     const targetDateStr = formData.dueDate || todayString;
//     const taskCost = getSelectedCategoryWeight();
//     const limit = user?.dailyEnergyLimit || 100;

//     const existingLoad = tasks.reduce((total, t) => {
//       if (t.isCompleted) return total;

//       const taskDate = t.dueDate ? t.dueDate.split("T")[0] : null;
//       if (taskDate === targetDateStr) {
//         const weight = Number(t.energyRequired) || Number(t.energyPoints) || 0;
//         return total + weight;
//       }
//       return total;
//     }, 0);

//     return existingLoad + taskCost > limit;
//   };
...
//     const isOverLimit = checkEnergyAllowance();
//     if (isOverLimit) {
//       setShowModal(true);
//     } else {
//       saveTask();
...
//   return (
//     <>
//       <form onSubmit={handlePreSubmit} className="p-2 bg-white">
//         <div className="mb-3">
...
//         show={showModal}
//         type="danger"
//         title="Energy Limit Warning"
//         message={`Adding this task will exceed your planned daily energy allowance for ${formData.dueDate || "today"}. Would you like to proceed anyway?`}
...
const AddTask = ({ onTaskAdded, user, tasks = [] }) => {
  ...
  const handlePreSubmit = (e) => {
    ...
    // 1. Check for rest day intersection first
    const targetDateStr = formData.dueDate || todayString;
    if (user?.holidays?.includes(targetDateStr)) {
      setShowRestModal(true);
      return;
    }

    // 2. Check energy allowance if it's not a rest day
    const isOverLimit = checkEnergyAllowance();
    if (isOverLimit) {
      setShowModal(true);
    } else {
  ...
  return (
    <>
      ...
      {/* Rest Boundary Warning (Now includes onConfirm to bypass) */}
      <CustomModal
        show={showRestModal}
        type="warning"
        title="Rest Boundary Warning"
        message={`You are scheduling a task on a designated rest day (${formData.dueDate || "today"}). Would you like to proceed anyway?`}
